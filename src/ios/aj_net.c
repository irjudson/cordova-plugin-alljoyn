...
AJ_Status AJ_Net_Send(AJ_IOBuffer* buf)
{
    ...
    if (tx > 0) {
        ret = send(context->tcpSock, buf->readPtr, tx, SO_NOSIGPIPE);
        if (ret == -1) {
...
static void sendToBroadcast(int sock, void* ptr, size_t tx)
{
    ...
    while (addr != NULL) {
        ...
        if (addr->ifa_addr != NULL && addr->ifa_addr->sa_family == AF_INET) {
            ...
            struct sockaddr_in* sin_bcast = (struct sockaddr_in*) addr->ifa_dstaddr;
            ...
            sendto(sock, ptr, tx, SO_NOSIGPIPE, (struct sockaddr*) sin_bcast, sizeof(struct sockaddr_in));
        }
    ...
    if (tx > 0) {
        if (context->udpSock != INVALID_SOCKET) {
            ...
            if (inet_pton(AF_INET, AJ_IPV4_MULTICAST_GROUP, &sin.sin_addr) == 1) {
                ret = sendto(context->udpSock, buf->readPtr, tx, SO_NOSIGPIPE, (struct sockaddr*)&sin, sizeof(sin));
                if (tx == ret) {
                    sendSucceeded = TRUE;
                }
            } else {
        ...
        if (context->udp6Sock != INVALID_SOCKET) {
            ...
            if (inet_pton(AF_INET6, AJ_IPV6_MULTICAST_GROUP, &sin6.sin6_addr) == 1) {
                ret = sendto(context->udp6Sock, buf->readPtr, tx, SO_NOSIGPIPE, (struct sockaddr*) &sin6, sizeof(sin6));
                if (tx == ret) {
                    sendSucceeded = TRUE;
                }
            } else {
