...
static AJ_Status AJ_Net_Send(AJ_IOBuffer* buf)
{
    ...
    if (tx > 0) {
        ret = send((SOCKET)buf->context, (char*)buf->readPtr, tx, 0);
        if (ret == SOCKET_ERROR) {
...
static AJ_Status AJ_Net_Recv(AJ_IOBuffer* buf, uint32_t len, uint32_t timeout)
{
    ...
    rx = min(rx, len);
    if (rx) {
		DWORD ret = recv((SOCKET)buf->context, (char*)buf->writePtr, rx, 0);
    ...
    if (tx > 0) {
        ...
        for (i = 0; i < NumMcastSocks; ++i) {
            ...
            if (family == AF_INET6) {
				...
				ret = sendto(sock, (char*)buf->readPtr, tx, 0, (struct sockaddr*) &sin6, sizeof(struct sockaddr_in6));
                if (ret == SOCKET_ERROR) {
