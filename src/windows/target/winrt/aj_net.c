...
)
{
	...
	PWSTR pAddrString[MAX_ADDR_LEN];
	size_t nConvertedChars = 0;
	mbstowcs_s(&nConvertedChars, (wchar_t*)pAddrString, MAX_ADDR_LEN, pszAddrString, _TRUNCATE);

	if (Family == AF_INET)
	{
		SOCKADDR_IN sockaddress4;
		INT size = sizeof(sockaddress4);
		sockaddress4.sin_family = AF_INET;

		if (WSAStringToAddressW((wchar_t*)pAddrString, AF_INET, NULL, (LPSOCKADDR)&sockaddress4, &size) == 0)
		{
			memcpy(pAddrBuf, &sockaddress4.sin_addr, sizeof(sockaddress4.sin_addr));
			return 1;
		}
	}
	else if (Family == AF_INET6)
	{
		SOCKADDR_IN6_LH sockaddress6;
		INT size = sizeof(sockaddress6);
		sockaddress6.sin6_family = AF_INET6;

		if (WSAStringToAddressW((wchar_t*)pAddrString, AF_INET6, NULL, (LPSOCKADDR)&sockaddress6, &size) == 0)
		{
			memcpy(pAddrBuf, &sockaddress6.sin6_addr, sizeof(sockaddress6.sin6_addr));
			return 1;
		}
	}
...
// Phong Added for emulator (Debugging purpose) - ws2tcpip.h
...
AJ_Status AJ_Net_Connect(AJ_NetSocket* netSock, uint16_t port, uint8_t addrType, const uint32_t* addr)
{
    ...
    if (ret == SOCKET_ERROR) {
		...
		sendEvent = CreateEventEx(NULL, NULL, CREATE_EVENT_MANUAL_RESET, MAXIMUM_ALLOWED);
		recvEvent = CreateEventEx(NULL, NULL, CREATE_EVENT_MANUAL_RESET, MAXIMUM_ALLOWED);
		interruptEvent = CreateEventEx(NULL, NULL, CREATE_EVENT_MANUAL_RESET, MAXIMUM_ALLOWED);
        wsaOverlapped.hEvent = INVALID_HANDLE_VALUE;
        return AJ_OK;
    }
}
        ...
        for (i = 0; i < NumMcastSocks; ++i) {
            ...
            if ((buf->flags & AJ_IO_BUF_AJ) && !McastSocks[i].is_mdns) {
                ...
                if (family == AF_INET && McastSocks[i].has_mcast4) {
					...
					ret = sendto(sock, (char*)buf->readPtr, tx, 0, (struct sockaddr*) &sin, sizeof(struct sockaddr_in));
                    if (ret == SOCKET_ERROR) {
                ...
                if (family == AF_INET && McastSocks[i].v4_bcast.s_addr) {
					...
					ret = sendto(sock, (char*)buf->readPtr, tx, 0, (struct sockaddr*) &bsin, sizeof(struct sockaddr_in));
                    if (ret == SOCKET_ERROR) {
...
static void Mcast6Up(const char* group, uint16_t port, uint8_t mdns, uint16_t recv_port)
{
	...
	// Phong TODO: Adding support for Windows Store apps
