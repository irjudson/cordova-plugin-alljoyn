...
typedef struct _AuthContext {
	...
	AllJoynWinRTComponent::AJ_PeerAuthenticateCallback^ callback; /* Callback function to report completion */
#else
...
static void HandshakeComplete(AJ_Status status)
{
	...
	{
#if (WINAPI_FAMILY == WINAPI_FAMILY_PC_APP || WINAPI_FAMILY == WINAPI_FAMILY_PHONE_APP)
		authContext.callback((uint8_t)status);
#else
...
#if (WINAPI_FAMILY == WINAPI_FAMILY_PC_APP || WINAPI_FAMILY == WINAPI_FAMILY_PHONE_APP)
AJ_Status AJ_PeerAuthenticate(AJ_BusAttachment* bus, const char* peerName, AllJoynWinRTComponent::AJ_PeerAuthenticateCallback^ callback)
#else
...
#if (WINAPI_FAMILY == WINAPI_FAMILY_PC_APP || WINAPI_FAMILY == WINAPI_FAMILY_PHONE_APP)
	AJ_InfoPrintf(("PeerAuthenticate(bus=0x%p, peerName=\"%s\", callback=0x%p)\n",
