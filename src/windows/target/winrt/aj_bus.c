...
void AJ_BusSetPasswordCallback(AJ_BusAttachment* bus, AllJoynWinRTComponent::AJ_AuthPwdFunc^ pwdCallback)
#else
...
#if (WINAPI_FAMILY == WINAPI_FAMILY_PC_APP || WINAPI_FAMILY == WINAPI_FAMILY_PHONE_APP)
AJ_Status AJ_BusAuthenticatePeer(AJ_BusAttachment* bus, const char* peerName, AllJoynWinRTComponent::AJ_PeerAuthenticateCallback^ callback)
{
	AJ_InfoPrintf(("AJ_BusAuthenticatePeer(bus=0x%p, peerName=\"%s\", callback=0x%p)\n", bus, peerName, callback));
	return AJ_PeerAuthenticate(bus, peerName, callback);
}
#else
